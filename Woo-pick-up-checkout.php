...
/**
 * Plugin Name: click and collect du Bdc Crew
 * Description: Customize the checkout page with date & time.
 * Author:      Le Bdc Crew t'as vu !
 * Version:     1.0.0
 * Author URI:  https://brutdecom.fr
...
function pick_up_checkout_field($checkout)
{
    ...
    woocommerce_form_field('order_pick_up_time', array(
        ...
        'label' => __('heure de Retrait'),
        'required' => get_option('pick_up_checkout_plugin_is_optional', 'true') !== 'true',
        'options' => array(
            '' => __(''),
            '11AMto2PM' => __('entre 11h et 14h '),
            '2PMto5PM' => __(' entre 14h et 17h'),
            '5PMto9PM' => __('entre 17h et 19H')
        ),
        'default' => '',
...
function pick_up_checkout_field_process()
{
    ...
    if (!$_POST['order_pick_up_date'])
        wc_add_notice(__('Merci ce selectionner une date de retrait.'), 'error');

    if (!$_POST['order_pick_up_time'])
        wc_add_notice(__('Merci ce selectionner une heure de retrait.'), 'error');
}
